		...
		return {
			...
			controller: function($scope, $element) {
				...
				$("#" + options.element + " svg").css({
					height: "100%",
					width: "100%"
				}), paper.setViewBox(0, 0, radius, radius, !0), 
				paper.canvas.setAttribute("preserveAspectRatio", "none"), 
				paper.customAttributes.arc = function(xloc, yloc, value, total, R) {
					var path, alpha = 360 / total * value,
					a = (90 - alpha) * Math.PI / 180,
					x = xloc + R * Math.cos(a),
					y = yloc - R * Math.sin(a);
					return path = total === value ? [
						["M", xloc, yloc - R],
						["A", R, R, 0, 1, 1, xloc - .01, yloc - R]
					] : [
						["M", xloc, yloc - R],
				...
				var counter = function(el, n, max) {
					...
					}()*//*Закоментировано для отмены отрисовки поля с процентами под графиком*/
				},
				...
				*//*Закоментировано для отмены предварительной отрисовки текста внутри графика*/
				var newArc = !1,
				newArcBg = !1;

				/*Добавлено для отслеживания изменения поля value для обновления названия графика*/
				$scope.$watch('config.value', function() {
					options.text = $scope.config.value;
					if (options.text) {
						paper.text(radius / 2, radius / 2, options.text += '%').attr({
							"font-size": radius / 4
						})
				...
				$scope.$watch('config.name', function() {
					options.name = $scope.config.name;
				...
				$scope.$watch("config.value", function() {
					...
					newArc.hover(function() {
						options.opacity && newArc.animate({
							"stroke-opacity": "1"
						}, 200)}, 
						function() {
							options.opacity && newArc.animate({
					...
					newArc.rotate(0, 100, 100).animate({
						arc: [radius / 2, radius / 2, options.value, 100, .425 * radius]
					}, 
					options.duration, options.easing), options.name) {
